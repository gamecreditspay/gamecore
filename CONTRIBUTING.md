...
These are some global design goals in gamecore that any change must adhere.
...
When possible, gamecore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Gamecore methods. For example, calling `JSON.stringify` on an Gamecore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'gamecore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the gamecore object.

DO:
```javascript
var gamecore = require('../');
var PublicKey = gamecore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add litecoin-project git@github.com:litecoin-project/gamecore.git
...
```
Finally go to [github.com/litecoin-project/gamecore](https://github.com/litecoin-project/gamecore) in your web browser and issue a new pull request.
...
[github.com/litecoin-project/gamecore/issues](https://github.com/litecoin-project/gamecore/issues).
